...
export function CalendarView() {
  ...
  const getDayContent = (day: Date | undefined) => {
    ...
    const dayEvents = events.filter((event) => isValid(event.date) && isSameDay(event.date, day))
    const hasApplications = dayEvents.some((event) => event.type === "application")
    const hasFollowUps = dayEvents.some((event) => event.type === "follow-up")

    return (
      <>
        <time dateTime={format(day, "yyyy-MM-dd")}>{format(day, "d")}</time>
        <div className="absolute bottom-0 left-0 right-0 flex justify-center gap-0.5">
          {hasApplications && <div className="h-1 w-1 rounded-full bg-blue-500" />}
          {hasFollowUps && <div className="h-1 w-1 rounded-full bg-yellow-500" />}
        </div>
      </>
    )
  }
            ...
            <CardContent className="p-4">
              {isLoading ? (
                ...
                <Calendar
                  ...
                  components={{
                    Day: (props) => {
                      // Just return the default day component with our content
                      return <props.Component {...props}>{props.day && getDayContent(props.day)}</props.Component>
                    },
                  }}
                />
              )}
